---
title: "Lowest Rated 100 Movies in IMDB"
...
```{r}
link <- "https://www.imdb.com/chart/bottom/?ref_=chtbo_ql_7"
```
...
Low_100_website <- link %>% read_html()
```
...
Low_100_website <- content(response, "text") %>% read_html()

# Select the movie title nodes
movie_title_nodes <- html_nodes(Low_100_website, "h3.ipc-title__text")
...
movie_titles <- html_text(movie_title_nodes)[2:101]
print(movie_titles)

```
URL
```{r}
movie_link_nodes <- html_nodes(Low_100_website, 'a.ipc-title-link-wrapper')
movie_urls <- html_attr(movie_link_nodes, 'href')

full_movie_urls <- paste0("https://www.imdb.com", movie_urls)
full_movie_urls <- full_movie_urls[1:100]
...
```{r}
url <- "https://www.imdb.com/title/tt1530509/?ref_=chtbtm_t_100" 
...
Excel
```{r}
write.csv(movies_df_final, file = "movies_data_lowest_100.csv", row.names = FALSE)
```
