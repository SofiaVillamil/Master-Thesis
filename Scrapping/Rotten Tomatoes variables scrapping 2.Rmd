...
```{r}
data <- read_csv("C:/Users/sofia/Documents/Documentos/Master Computational Social Sciences/TFM/Master Thesis/Data/movies_final_dataset_test.csv",show_col_types = FALSE)
```
...
generate_movie_url <- function(movie_title, release_year = NULL, known_variation = NULL) {
  if (!is.null(known_variation)) {
    # known variation used to account for differences
  ...
  # Function to check if the URL is valid so I can adjust the code
  ...
  # If release year is provided, generate the URL with year as well as this is an important variation
  ...
  # If no valid URL is found, generate URLs for multiple years and select the most recent one
  years <- 2010:2024
...
# Extract the required information with tryCatch so I can observe any errors if they appear
    data_scrap <- tryCatch({
...
plan(multisession, workers = 7)  #Ajusted the number to 7 as I have 8 CPU's
...
```{r}
# Save merged dataset
write_csv(final_data2, "C:/Users/sofia/Documents/Documentos/Master Computational Social Sciences/TFM/Master Thesis/Data/movies_test_dataset_with_rotten_tomatoes.csv")
```
