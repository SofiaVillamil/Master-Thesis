...
We need more variables to get a better rating for each actor, becuase I believe we need specific information about each actor not just for each movie. We are going to go back to scrapping IMDB using the same code but for an actor now.
...
```{r}
write.csv(nombres, "actors_with_imdb_ids_2.csv", row.names = FALSE)
```
...
```{r}
# Function to scrape IMDb actor page
...
# Merge the scraped data with the original nombres dataframe
final_df <- nombres3 %>%
...
write.csv(final_df_2, "actors_with_imdb.csv", row.names = FALSE)
