...
selected_data2 <- data_final %>%
  ...
  mutate(
    ...
    marketing = 0.5 * Budget, # it said it on the internet
...
# Normalize wins and nominations to ensure that many small awards don't overshadow big ones. I want the importan awards to still have weight.
...
# Merge the normalized director score back into the original data_final
data_final <- data_final %>%
...
```{r}
# Filter rows where director_is_a_woman is NA
...
```{r}
# List of women directors based on the provided by the chat that looked into 600 names
women_directors <- c(
...
```{r}
# List of women actors based on the provided names
women_actors <- c(
...
```{r}
write.csv(data_final2, "C:/Users/sofia/Documents/Documentos/Master Computational Social Sciences/TFM/Master Thesis/Modeling Part 3/data_final_6.csv")
