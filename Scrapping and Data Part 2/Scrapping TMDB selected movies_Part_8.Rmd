...
This part needs to be done in sets of titles so we do not affect the website. I did it in chunks of 300 movies. You start with is 1:300 and continue on till you reach the last observations
...
```{r}
data <- data_final_4 # so i dont affect previous code for the urls
```

```{r}
# as we are doing it in chunck, it will be manually updated so it does not overwrite previous observations
...
### Saving
```{r}
write.csv(merged_data, "C:/Users/sofia/Documents/Documentos/Master Computational Social Sciences/TFM/Master Thesis/Scrapping and Data Part 2/data_final_5.csv",row.names = FALSE)

```

This is the last scrapping script. Now we move on to the Modeling Part 3 file where we will start with the script "Creating the variables_Part_1"
