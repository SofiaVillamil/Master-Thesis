---
title: "Instagram Scrapping_Part_4"
...
We will attain a variable for the follower that each actor has. However, first to reduce the scrapping amount we will clean the data eliminating the movies with a lot of NA's.
...
### Data set samples for Scrapping in Instagram

```{r}
movies2 <- data_filtered_3 %>% select(FirstActor_imdb,imdb_id)
movies2 <- movies2 %>% distinct(FirstActor_imdb, .keep_all = TRUE)
write.csv(movies2, "C:/Users/sofia/Documents/Documentos/Master Computational Social Sciences/TFM/Master Thesis/Scrapping and Data Part 2/data_final_actores.csv",row.names = FALSE)

```

```{r}
# Selecting now a data set with all the actors
movies3 <- data_filtered_3 %>% select(imdb_id,FirstActor_imdb, SecondActor_imdb)

nombres <- movies3 %>%
  pivot_longer(cols = -imdb_id, names_to = "type", values_to = "nombre") %>%
  select(imdb_id, nombre) %>%
  distinct()

write.csv(nombres, "C:/Users/sofia/Documents/Documentos/Master Computational Social Sciences/TFM/Master Thesis/Scrapping and Data Part 2/data_final_actores_full.csv",row.names = FALSE)
